...
describe("<TaskForm />", () => {
    ...
    test("render description input", () => {
        ...
        const descriptionElement = screen.getByTestId(
            "adding-task-form-input-description"
        );
    ...
    test("Form successfully submitted filled in", () => {
        ...
        fireEvent.change(screen.getByTestId("adding-task-form-input-title"), {
            target: { value: inputValue1 },
        });
        fireEvent.change(
            screen.getByTestId("adding-task-form-input-description"),
            { target: { value: inputValue2 } }
        );
        fireEvent.click(screen.getByText(/Add Task/i));
        expect(logSpy).toHaveBeenCalledTimes(1);
        expect(logSpy).toHaveBeenCalledWith(
            `Submitting Name ${inputValue1} & ${inputValue2}`
        );
    });
